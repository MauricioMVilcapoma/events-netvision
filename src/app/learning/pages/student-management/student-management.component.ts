...
export class StudentManagementComponent implements OnInit, AfterViewInit  {
  ...
  displayedColumns: string[] = ['event_id', 'name', 'date', 'description', 'location', 'capacity', 'organizer_id', 'actions'];
  isEditMode: boolean;
  ...
  private updateStudent(): void {
    ...
    this.studentService.update(this.studentData.event_id, studentToUpdate)
      .subscribe((response: any) => {
        this.dataSource.data = this.dataSource.data
          .map((student: Student) => {
            if (student.event_id === response.id) {
              return response;
            }
            return student;
          });
      ...
      .subscribe(() => {
        this.dataSource.data = this.dataSource.data
          .filter((student: Student) => {
            return student.event_id !== studentId ? student : false;
          });
      });
  };
  ...
  onDeleteItem(element: Student) {
    this.deleteStudent(element.event_id);
  }
